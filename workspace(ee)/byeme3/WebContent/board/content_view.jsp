...
<%
	...
	int strId = bId;
	boardDto dto = new boardDao().contentView(strId);
%>

<c:set var="board" value="${requsetScope.bId }"/>
	...
	<div class="container">
		<div class="row">
			...
			<tbody>
				<tr>
					<td width="130" align="center">${content_view.userId }</td>
					<td width="1">|</td>
					<td width="700" align="left">${content_view.bTitle }</td>
					<td width="1">|</td>
					<td width="75" align="left"> 조회 : ${content_view.bHit }</td>
					<td width="1">|</td>
					<td width="160">${content_view.bDate }</td>
				</tr>
				<tr>
				</tr>
				<tr>	
					<td colspan="8" style="height: 350px; vertical-align: middle;" align="left" >${fn:replace(content_view.bContent, cn, br) }</td>
				</tr>
			</tbody>
			</table>
			</form>
	...
	<c:if test="${requestScope.commentList != null}">
		<c:forEach var="comment" items="${requestScope.commentList}">
            ...
            <form id='writeCommentForm'>
                <input type="hidden" name="comment_board" value="<%= bId %>">
			...
			%>
					<a href="modify.jsp?bId=<%= bId %>">수정</a> |
						
					<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="delete.do?bId=<%= bId %>">삭제</a>
			<%
				}
			%>
			<%
			...
			%>
				...
				<a href="reply.jsp?bId=<%= bId %>" class="btn btn-primary pull-right" style="margin-right: 5px">답변</a>
			<%
				}
			%>
			</form>
